...
function saveReview($review_data) {
    if (is_user_logged_in()) {
        $post_author = get_current_user_id();
    } else {
        if ($review_data["user_email"] && get_user_by("email", $review_data["user_email"])) {
            $post_author = get_user_by("email", $review_data["user_email"])->ID;
        } else {
            $post_author = -1;
        }
    }
    $date = new DateTime('now');
    $post_title = str_replace(":", "", str_replace("-", "", str_replace(" ", "", "Review" . $date->format('Y-m-d H:i:s') . $date->getTimestamp())));
    $post_args = array(
        'post_title' => wp_strip_all_tags($post_title),
        'post_type' => 'reviews_cpt',
        'post_content' => $review_data["review_content"],
        'post_author' => $post_author,
        'post_status' => 'publish',
        'meta_input' => array(
            'review-user-email' => $review_data["user_email"],
            'review-user-mark' => $review_data["review_mark"],
            'review-user-name' => $review_data["user_name"],
            'post-ID' => $review_data["post_id"]
        )
    );
    $review_id = wp_insert_post($post_args, true);
    return $review_id;
}

function send_email_new_review_for_book($review_id) {
    $headers[] = 'MIME-Version: 1.0';
    $headers[] = 'Content-Type: text/html; charset=UTF-8';
    $headers[] = 'From: BledShare <infos@bledshare.com>';
    $headers[] = 'Bcc:<erictonyelissouck@yahoo.fr>';

    $sender_email = get_post_meta($review_id, "review-user-email", true);
    $sender_phone_number = get_post_meta($review_id, "review-user-phone-number", true);
    $sender_user_name = get_post_meta($review_id, "review-user-name", true);
    $post_referred_id = get_post_meta($review_id, "post-ID", true);
...
function send_email_new_review_for_alert($review_id) {
    $headers[] = 'MIME-Version: 1.0';
    $headers[] = 'Content-Type: text/html; charset=UTF-8';
    $headers[] = 'From: BledShare <infos@bledshare.com>';
    $headers[] = 'Bcc:<erictonyelissouck@yahoo.fr>';
...
function send_email_review_reply_for_book($review_id) {
    $headers[] = 'MIME-Version: 1.0';
    $headers[] = 'Content-Type: text/html; charset=UTF-8';
    $headers[] = 'From: BledShare <infos@bledshare.com>';
    $headers[] = 'Bcc:<erictonyelissouck@yahoo.fr>';
...
function send_email_review_reply_for_alert($review_id) {
    $headers[] = 'MIME-Version: 1.0';
    $headers[] = 'Content-Type: text/html; charset=UTF-8';
    $headers[] = 'From: BledShare <infos@bledshare.com>';
    $headers[] = 'Bcc:<erictonyelissouck@yahoo.fr>';
...
function send_sms_review_reply_for_book($review_id){
    $sender_phone_number = get_post_meta($review_id, "review-user-phone-number", true);
    $sender_user_name = get_post_meta($review_id, "review-user-name", true);
    $post_referred_id = get_post_meta($review_id, "post-ID", true);
    $post_referred_user = get_user_by('id', get_post_field('post_author', $post_referred_id));
...
function send_sms_review_reply_for_alert($review_id){
    $sender_phone_number = get_post_meta($review_id, "review-user-phone-number", true);
    $sender_user_name = get_post_meta($review_id, "review-user-name", true);
    $post_referred_id = get_post_meta($review_id, "post-ID", true);
    $post_referred_user = get_user_by('id', get_post_field('post_author', $post_referred_id));
...
//Function for adding a comment to a review 
function add_review_comment($review_id, $comment_content) {
    global $current_user;
    $comment_id = null;
    if ($review_id) {
        $commentdata = array(
            'comment_post_ID' => $review_id, // to which post the comment will show up
...
function getAndechoAllReviewComments($review_id) {
    global $current_user;
    $comments_view_content = "";
    if ($review_id) {
        $comments = get_comments(array('post_id' => $review_id, "parent" => 0, "orderby" => "comment_date", "order" => "asc"));
