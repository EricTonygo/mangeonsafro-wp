...
<section class="hero">
    <div class="container">
        ...
        <ol class="breadcrumb justify-content-center">
            ...
            <li class="breadcrumb-item"><a href="<?php echo esc_url(add_query_arg(array('shop-id' => $shop_id), wp_make_link_relative(get_permalink(get_page_by_path(__('account', 'mangeonsafrodomain') . "/" . __('seller-zone', 'mangeonsafrodomain') . "/" . __('products', 'mangeonsafrodomain'))->ID)))) ?>"><?php _e("Product of ", "mangeonsafrodomain"); ?> <?php echo $prod_shop->post_title; ?></a></li>
        ...
        <div class="row">
            <div class="col-lg-8 col-xl-9">
                <div class="block mb-5">
                    <div class="block-header"><strong class="text-uppercase"><?php _e("Edit product", "mangeonsafrodomain"); ?></strong></div>
                    <div class="block-body">
                        <?php include(locate_template("statics-pages/content-success-or-faillure-message.php")); ?>
                        <form method="POST" action="<?php echo wp_make_link_relative(get_permalink()) ?>" enctype="multipart/form-data" autocomplete="off">
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        <label for="product-currency-id" class="form-label"><?php _e("Product's currency", "mangeonsafrodomain"); ?></label>                                        
                                        <select id="product-currency-id" name="product-currency-id" class="form-control">
                                            <option value=""><?php _e("Select a product currency", "mangeonsafrodomain"); ?></option>
                                            <?php
                                            ...
                                            if ($currencies->have_posts()) :
                                                ...
                                                while ($currencies->have_posts()): $currencies->the_post();
                                                    ?>
                                                    <option value="<?php the_ID(); ?>" <?php if (get_the_ID() == $prod_currency->ID): ?> selected="selected" <?php endif ?>><?php the_title() ?></option>
                                                    <?php
                                                endwhile;
                                                ?>
                                                <?php
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="product-category" name="product-category" class="form-control" required="true">
                                            ...
                                            foreach ($product_parent_categories as $product_parent_category):
                                                ?>
                                                <option value="<?php echo $product_parent_category->slug; ?>" <?php if (is_array($product_categories) && in_array($product_parent_category->term_id, $product_categories, true)): ?> selected="selected" <?php endif ?>><?php echo __($product_parent_category->name, "mangeonsafrodomain"); ?></option>
                                            <?php endforeach ?>
...
//                                            $product_parent_categories = get_categories(array('hide_empty' => false, 'orderby' => 'name', 'order' => 'ASC', 'parent' => 0));
                                            foreach ($product_parent_categories as $product_parent_category):
                                                ...
                                                <optgroup label="<?php echo $product_parent_category->name; ?>">
                                                    ...
                                                    foreach ($product_sub_categories as $product_sub_category):
                                                        ?>
                                                        <option value="<?php echo $product_sub_category->slug; ?>" <?php if (is_array($product_categories) && (in_array($product_sub_category->slug, $product_categories, true) || in_array($product_sub_category->term_id, $product_categories, true))): ?> selected="selected" <?php endif ?>><?php echo $product_sub_category->name; ?></option>
                                                    <?php endforeach ?>
                                                </optgroup>
                                            <?php endforeach ?>
                            ...
                            <div class="row">
                                <div class="col-sm-12">
                                    <div class="form-group">
                                        ...
                                        if ($prod_thumbnail_image_id):
                                            ?>
                                            <input id="product-thumbnail-image-id" type="hidden" name="product-thumbnail-image-id" value="<?php echo $prod_thumbnail_image_id; ?>">
                                        <?php endif ?>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-sm-12">
                                    <div class="form-group">
                                        <label for="product-description" class="form-label"><?php _e("Product description", "mangeonsafrodomain"); ?></label>
                                        <textarea id="product-description" class="form-control" rows="5" name="product-description" placeholder="<?php _e("Product description", "mangeonsafrodomain"); ?>" ><?php echo $prod_description; ?></textarea>
                                    </div>
                                </div>
                            </div>
                            <!-- /.row-->
